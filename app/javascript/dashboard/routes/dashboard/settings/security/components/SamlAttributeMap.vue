...
<template>
  <section
    class="rounded-xl border border-n-border-glass-soft bg-n-glass-soft w-full text-sm text-n-text-display mb-5 overflow-hidden"
  >
    <button
      type="button"
      class="w-full px-4 py-3 flex items-center justify-between text-left hover:bg-n-glass-strong transition-colors"
      @click="toggleExpanded"
    >
      <h4 class="font-medium text-n-text-display">
    ...
    >
      <div class="px-4 pb-3">
        <p class="text-n-text-body mb-2">
          {{ t('SECURITY_SETTINGS.SAML.ATTRIBUTE_MAPPING.DESCRIPTION') }}
        </p>
        <!-- eslint-disable vue/no-bare-strings-in-template -->
        <ul class="list-none text-n-text-display space-y-1">
