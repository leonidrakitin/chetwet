...
const buttonLabel = computed(() => {
  if (selectedTeams.value.length === 0) {
    return t('TEAM_REPORTS.FILTER_DROPDOWN_LABEL');
  }
  if (selectedTeams.value.length === 1) {
    const team = options.value.find(o => o.id === selectedTeams.value[0].id);
    return team?.name || t('TEAM_REPORTS.FILTER_DROPDOWN_LABEL');
  }
  return (
    t('TEAM_REPORTS.FILTER_DROPDOWN_LABEL') + ` (${selectedTeams.value.length})`
  );
});

const isSelected = id => selectedTeams.value.some(tm => tm.id === id);

const toggle = item => {
  const idx = selectedTeams.value.findIndex(tm => tm.id === item.id);
  if (idx >= 0) {
    selectedTeams.value = selectedTeams.value.filter((_, i) => i !== idx);
  } else {
    selectedTeams.value = [
      ...selectedTeams.value,
  ...
  emit('teamFilterSelection', [...selectedTeams.value]);
};
