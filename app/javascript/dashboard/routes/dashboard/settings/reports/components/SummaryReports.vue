...
const defaulSpanRender = cellProps =>
  h(
    'span',
    {
      class: cellProps.getValue() ? '' : 'text-n-text-display',
    },
    cellProps.getValue()
  );
...
<template>
  ...
  />
  <div
    class="relative flex-1 overflow-auto px-2 py-2 mt-5 shadow outline-1 outline outline-n-border-glass-soft rounded-xl bg-n-glass-strong"
  >
    <Table :table="table" />
    <Transition
    ...
    >
      <div
        v-if="isLoading"
        class="absolute inset-0 flex justify-center pt-[12.5rem] bg-n-glass-soft/70 rounded-xl pointer-events-none"
      >
