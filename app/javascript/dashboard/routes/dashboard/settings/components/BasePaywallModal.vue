...
<template>
  <div
    class="flex flex-col max-w-md px-6 py-6 border shadow bg-n-glass-soft rounded-xl border-n-border-glass-soft"
  >
    <div class="flex items-center w-full gap-2 mb-4">
      ...
      <span class="text-base font-medium text-n-text-display">
        {{ $t(`${featurePrefix}.PAYWALL.TITLE`) }}
      </span>
    </div>
    <p
      v-dompurify-html="$t(`${featurePrefix}.${i18nKey}.AVAILABLE_ON`)"
      class="text-sm font-normal text-n-text-body"
    />
    <p class="text-sm font-normal text-n-text-body">
    ...
    <template v-if="isOnChatwootCloud">
      ...
      <span class="mt-2 text-xs tracking-tight text-center text-n-text-body">
