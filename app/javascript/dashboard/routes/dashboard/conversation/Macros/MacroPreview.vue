...
<template>
  <div
    class="macro-preview absolute border border-n-border-glass-soft max-h-[22.5rem] z-50 w-64 rounded-md bg-n-alpha-3 backdrop-blur-[100px] shadow-lg bottom-8 right-8 overflow-y-auto p-4 text-left rtl:text-right"
  >
    <h6 class="mb-4 text-sm text-n-text-display">
      {{ macro.name }}
    </h6>
    <div
    ...
    >
      ...
      />
      <div
        class="absolute -left-[0.21875rem] top-[0.2734375rem] w-2 h-2 rounded-full bg-n-glass-soft border-2 border-solid border-n-border-glass-soft dark:border-n-slate-6"
      />
      <p class="mb-1 text-xs text-n-text-body">
        {{ $t(`MACROS.ACTIONS.${action.actionName}`) }}
      </p>
      <p class="text-n-text-display text-sm">{{ action.actionValue }}</p>
    </div>
  </div>
</template>
