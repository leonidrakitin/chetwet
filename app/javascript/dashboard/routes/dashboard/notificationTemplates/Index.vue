...
const ROUTE_TYPE_MAP = {
  notification_templates_marketing: 'marketing',
  notification_templates_service: 'service',
...
const isTemplateList = computed(() => !isStatisticsTab.value);

const templateFilterType = computed(() => {
  if (activeType.value === 'marketing') return 'marketing';
  if (activeType.value === 'service') return 'service';
  return null;
});
...
const filteredTemplates = computed(() => {
  ...
  if (templateFilterType.value) {
    templates = templates.filter(
      tmpl => tmpl.template_category === templateFilterType.value
    );
  }
...
<template>
  ...
  >
    ...
    <!-- Content -->
    <div
      v-if="!uiFlags.isFetching && filteredTemplates.length === 0"
      class="flex-1 min-h-0 flex flex-col items-center justify-center h-full gap-4 py-12 overflow-y-auto px-4 py-3 md:px-6 md:py-4"
    >
      <div
    ...
    <Draggable
      ...
      animation="200"
      class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 flex-1 min-h-0 overflow-y-auto px-4 py-3 md:px-6 md:py-4"
    >
    ...
    <FlowMap
      ...
      class="flex-1 min-h-0 overflow-hidden min-w-0"
      @edit="handleEdit"
    />
  </div>
