...
<template>
  <div class="flex flex-col gap-2 border-b border-n-border-glass group/note">
    <div class="flex items-center justify-between gap-2">
      <div class="flex items-center gap-1.5 min-w-0">
        ...
        <div class="min-w-0 truncate">
          <span class="inline-flex items-center gap-1 text-sm text-n-text-body">
            <span class="font-medium text-n-text-display">{{ writtenBy }}</span>
            {{ t('CONTACTS_LAYOUT.SIDEBAR.NOTES.WROTE') }}
            <span class="font-medium text-n-text-display">
    ...
    <p
      ...
      class="mb-0 prose-sm prose-p:text-sm prose-p:leading-relaxed prose-p:mb-1 prose-p:mt-0 prose-ul:mb-1 prose-ul:mt-0 text-n-text-display"
      :class="{
