...
<template>
  <li class="n-dropdown-item">
    <component
      :is="componentIs"
      v-bind="$attrs"
      class="flex text-left rtl:text-right items-center p-2 reset-base text-sm text-n-text-display w-full border-0"
      :class="{
    ...
    >
      <slot>
        <slot name="icon">
          <Icon v-if="icon" class="size-4 text-n-text-body" :icon="icon" />
        </slot>
