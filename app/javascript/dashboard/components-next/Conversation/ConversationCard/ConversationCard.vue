...
<template>
  <div
    ...
    @click="onCardClick"
  >
    <div v-tooltip.left="inboxName" class="flex-shrink-0">
      <Avatar
    ...
    <div class="flex flex-col w-full gap-1 min-w-0">
      <div class="flex items-center justify-between h-6 gap-2">
        <h4 class="text-base font-medium truncate text-n-text-display">
          {{ currentContactName }}
        </h4>
        <div class="flex items-center gap-2">
          <CardPriorityIcon :priority="conversation.priority || null" />
          <span class="text-sm text-n-text-body/60">{{ lastActivityAt }}</span>
        </div>
      </div>
      <CardMessagePreview
