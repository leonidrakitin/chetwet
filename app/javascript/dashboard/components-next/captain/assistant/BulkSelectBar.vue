...
<template>
  ...
  >
    <div
      v-if="hasSelected"
      class="flex items-center gap-3 py-1 ltr:pl-3 rtl:pr-3 ltr:pr-4 rtl:pl-4 rounded-lg bg-n-glass-strong outline outline-1 outline-n-border-glass-soft shadow"
    >
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-1.5 min-w-0">
          ...
          />
          <span
            class="text-sm font-medium truncate text-n-text-display tabular-nums"
          >
            {{ selectAllLabel }}
          </span>
        </div>
        <span class="text-sm text-n-text-body/60 truncate tabular-nums">
          {{ selectedCountLabel }}
        </span>
