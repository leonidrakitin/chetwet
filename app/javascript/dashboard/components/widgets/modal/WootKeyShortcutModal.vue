    ...
    <div class="flex flex-col h-auto overflow-auto">
      ...
      <div class="grid grid-cols-2 px-8 pt-0 pb-4 mt-6 gap-x-5 gap-y-3">
        <div class="flex justify-between items-center min-w-[25rem]">
          <h5 class="text-sm text-n-text-display">
      ...
      <div class="grid grid-cols-2 px-8 pt-0 pb-8 gap-x-5 gap-y-3">
        ...
        >
          <h5 class="text-sm text-n-text-display min-w-[36px]">
            {{ title(shortcut) }}
          </h5>
          <div class="flex items-center gap-2 mb-1 ml-2">
            ...
            <template v-for="(key, index) in shortcut.displayKeys" :key="index">
              ...
              <span
                v-else
                class="flex items-center text-sm font-semibold text-n-text-display"
              >
                {{ key }}
              </span>
            </template>
...
<style scoped>
.key {
  @apply py-2 px-2.5 font-semibold text-xs text-n-text-display bg-n-glass-strong border border-n-border-glass-soft shadow-pill-soft rounded-pill backdrop-blur-glass-rail backdrop-saturate-glass;
}
</style>
