  ...
  <div class="flex flex-col p-3">
    <div class="w-12 h-12 mb-3">
      <img
        ...
        class="object-contain w-full h-full border rounded-md shadow-sm border-n-border-glass-soft dark:hidden dark:bg-n-alpha-2"
      />
      <img
        :src="`/dashboard/images/integrations/${integrationId}-dark.png`"
        class="hidden object-contain w-full h-full border rounded-md shadow-sm border-n-border-glass-soft dark:block"
      />
    </div>

    <div class="flex-1 mb-4">
      <h3 class="mb-1.5 text-sm font-medium text-n-text-display">
        {{ $t('INTEGRATION_SETTINGS.LINEAR.CTA.TITLE') }}
      </h3>
      <p v-if="isAdmin" class="text-sm text-n-text-body">
