...
<template>
  <div class="w-full">
    <div class="flex gap-2 mb-2.5">
      <div
        class="flex flex-1 gap-1 items-center px-2.5 py-0 rounded-lg bg-n-alpha-black2 outline outline-1 outline-n-border-glass-soft hover:outline-n-slate-6 dark:hover:outline-n-slate-6 focus-within:outline-n-brand dark:focus-within:outline-n-brand"
      >
        <fluent-icon icon="search" class="text-n-text-display" size="16" />
        <input
          v-model="query"
          type="search"
          :placeholder="t('WHATSAPP_TEMPLATES.PICKER.SEARCH_PLACEHOLDER')"
          class="reset-base w-full h-9 bg-transparent text-n-text-display !text-sm !outline-0"
        />
      </div>
      <button
        :disabled="isRefreshing"
        class="flex justify-center items-center w-9 h-9 rounded-lg bg-n-alpha-black2 outline outline-1 outline-n-border-glass-soft hover:outline-n-slate-6 dark:hover:outline-n-slate-6 hover:bg-n-alpha-2 dark:hover:bg-n-glass-strong disabled:opacity-50 disabled:cursor-not-allowed"
        :title="t('WHATSAPP_TEMPLATES.PICKER.REFRESH_BUTTON')"
        @click="refreshTemplates"
      >
        <Icon
          icon="i-lucide-refresh-ccw"
          class="text-n-text-display size-4"
          :class="{ 'animate-spin': isRefreshing }"
        />
      </button>
    </div>
    <div
      class="bg-n-background outline-n-border-glass-soft outline outline-1 rounded-lg max-h-[18.75rem] overflow-y-auto p-2.5"
    >
      <div v-for="(template, i) in filteredTemplateMessages" :key="template.id">
        <button
          class="block p-2.5 w-full text-left rounded-lg cursor-pointer hover:bg-n-alpha-2 dark:hover:bg-n-glass-strong"
        ...
        >
          <div>
            <div class="flex justify-between items-center mb-2.5">
              ...
              <span
                class="inline-block px-2 py-1 text-xs leading-none rounded-lg cursor-default bg-n-slate-3 text-n-text-display"
              >
            ...
            <div v-if="getTemplateHeader(template)" class="mb-3">
              <p class="text-xs font-medium text-n-text-body">
              ...
              <div
                ...
                class="text-sm italic text-n-text-body"
              >
                {{
            ...
            <div v-if="getTemplateButtons(template)" class="mt-3">
              ...
              <div class="flex flex-wrap gap-1 mt-1">
                <span
                  ...
                  :key="button.text"
                  class="px-2 py-1 text-xs rounded bg-n-slate-3 text-n-text-display"
                >
                  {{ button.text }}
                </span>
              </div>
        ...
        <hr
          ...
          :key="`hr-${i}`"
          class="border-b border-solid border-n-border-glass-soft my-2.5 mx-auto max-w-[95%]"
        />
      </div>
