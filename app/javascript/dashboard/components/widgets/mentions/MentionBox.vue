...
<template>
  <div
    ...
    class="bg-n-glass-soft p-1 rounded-xl overflow-auto absolute w-full z-20 shadow-md left-0 bottom-full max-h-[9.75rem] border border-solid border-n-border-glass mention--box"
  >
      ...
      >
        ...
        >
          <slot :item="item" :index="index" :selected="index === selectedIndex">
            <p
              class="max-w-full min-w-0 mb-0 overflow-hidden text-sm font-medium text-n-text-body group-hover:text-n-text-display text-ellipsis whitespace-nowrap"
              :class="{
                'text-n-text-display': index === selectedIndex,
              }"
            >
              {{ getPlainText(item.description) }}
            </p>
            <p
              class="max-w-full min-w-0 mb-0 overflow-hidden text-xs text-n-text-body group-hover:text-n-text-display text-ellipsis whitespace-nowrap"
              :class="{
                'text-n-text-display': index === selectedIndex,
              }"
            >
