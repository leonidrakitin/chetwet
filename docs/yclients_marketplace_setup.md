...
| Start Marketplace activation | POST | `https://<FRONTEND_URL>/api/v1/accounts/:account_id/yclients_marketplace/connect` (body: `{ "salon_ids": [123, 456] }`) |
...
5. When YClients sends `integration_revoked` (or similar), Chatwoot marks the integration as revoked. When it sends `integration_activated` with a token, Chatwoot can update the stored bearer token.
